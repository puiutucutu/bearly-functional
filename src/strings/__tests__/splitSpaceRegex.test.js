...
describe("splitSpaceRegex()", async assert => {
  assert({
    given: "a string",
    should: "return array",
    actual: splitSpaceRegex ("foo bar baz"),
  ...
  assert({
    ...
    actual: splitSpaceRegex ("hello   world"),
  ...
  assert({
    ...
    actual: Try(splitSpaceRegex) instanceof TypeError,
    expected: true
  });
});
